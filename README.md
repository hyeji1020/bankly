...
이는 JPA의 **1차 캐시**와 **영속성 컨텍스트의 동작 특성**으로 인해, 변경된 데이터가 동기화되지 않았기 때문입니다.
...
- **데이터 동기화 방법**: 네이티브 쿼리로 인해 발생한 데이터 불일치 문제를 해결하기 위해 다음과 같은 방법을 사용할 수 있음을 알게 되었습니다.
    - **영속성 컨텍스트 초기화**: `EntityManager.clear()`를 통해 1차 캐시를 초기화
    - **데이터 재조회**: 데이터베이스에서 변경된 값을 다시 가져와 동기화
    - **수동 값 설정**: 엔티티의 필드 값을 직접 갱신하여 데이터 동기화
